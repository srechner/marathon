...
#pragma omp parallel for if(omega > 1000)
	...
	T d = cpu::totalVariationDistance<T>(tmp[0], pi, dist);

	while (d >= epsilon) {
		tmp[1].mult(tmp[0], tmp[0]);
		tmp[0].swapContent(tmp[1]);
		d = cpu::totalVariationDistance<T>(tmp[0], pi, dist);
		l = r;
		r *= 2;
	}
	...
	while (l < r - 1) {
		...
		T dist_m = cpu::totalVariationDistance<T>(tmp[0], pi, dist);
 ...
 */
template int totalMixingTime<float>(const StateGraph*, const float);
template int totalMixingTime<double>(const StateGraph*, const double);
